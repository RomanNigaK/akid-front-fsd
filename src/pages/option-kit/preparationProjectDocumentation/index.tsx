...
export const PreparationProjectDocumentation = observer(({ kitId }: Props) => {
  const {
    person: {
      person,
      getPersonAction,
  ...
  useEffect(() => {
    getPersonAction(kitId, TypePerson.preparationProjectDocumentation);
  }, []);

  person?.case({
    pending: () =>
      message.loading({
        content: 'Загружаем данные лица, осуществляющего проектирование',
        duration: 0.5,
      }),
  });
  ...
  return (
    <>
      <FormPerson
        type={TypePerson.preparationProjectDocumentation}
        kitId={kitId}
